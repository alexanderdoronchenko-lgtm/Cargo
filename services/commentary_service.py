...
async def generate_moment_explanations(
    ...
    user_prompt = (
        ...
        "комментарий тренера, а не механический вердикт «ошибка/не ошибка». Как и в "
        "примерах выше, можно сначала коротко отметить, что было хорошо в позиции "
        "или замысле перед сбоем, прежде чем объяснить сам промах — но объяснение "
        "должно оставаться правдивым: перед тобой реальная ошибка (потеря более 100 "
        "сантипешек), не выдумывай похвалу самому ходу. Это будет "
